...
def _diagnosis_from_synthesis(result: dict, evidence: list[dict]) -> dict:
    ...
    if not findings:
        return {"status": result.get("status", "NO_ISSUE"), "root_cause": result.get("summary", "No verified operational issue was found."), "explanation": result.get("summary", "No verified operational issue was found."), "fix": "No remediation generated during investigation.", "kubectl_command": "", "prevention": "", "confidence": 0.0, "affected_resources": [], "findings": [], "evidence": []}
    primary, extra = findings[0], findings[1:]
    explanation = primary.get("explanation") or primary.get("root_cause") or ""
    if extra:
        explanation += " Additional independent findings: " + " ".join(str(f.get("root_cause") or f.get("explanation") or "") for f in extra[:4])
    ...
    return {"status": result.get("status", "DIAGNOSED"), "root_cause": primary.get("root_cause") or result.get("summary", ""), "explanation": explanation, "fix": "No remediation generated during investigation.", "kubectl_command": "", "prevention": "", "confidence": float(primary.get("confidence", 0.0) or 0.0), "affected_resources": primary.get("affected_resources") or [], "findings": findings, "evidence": selected_evidence}


def run_investigation(progress_callback: Callable[[str], None] | None = None, context: str | None = None, incident_description: str | None = None) -> dict:
    ...
    synthesis = validate_diagnosis(synthesize(verified, incident_description or DEFAULT_INCIDENT), verified)
    diagnosis = _diagnosis_from_synthesis(synthesis, verified)
    if progress_callback:
        progress_callback("Root Cause Found")

    return {"pods": {}, "logs": {}, "events": {}, "deployments": {}, "network": {"signals": operational_evidence}, "operational_evidence": operational_evidence, "security_evidence": [e.model_dump(mode="json") for e in security_evidence], "security_summary": security_summary, "diagnosis": diagnosis, "remediation_plan": None, "trace": [], "signals": operational_evidence}
