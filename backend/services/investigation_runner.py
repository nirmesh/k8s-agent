...
def run_and_save(investigation_id: str, context: str | None = None) -> None:
    ...
    try:
        ...
        security_precomputed = {"security_evidence": security_evidence_json, "security_summary": security_summary}
        security_evidence_count = persist_security_evidence(db, investigation_id, security_evidence)
        now = datetime.now(timezone.utc)
        db.investigations.update_one(
            {"_id": ObjectId(investigation_id)},
            {
                "$set": {"security_evidence_count": security_evidence_count, "security_summary": security_summary, "security_scan_completed": True, "updated_at": now},
                "$pull": {"steps": {"name": "Security Scan"}},
            },
        )
        db.investigations.update_one(
            {"_id": ObjectId(investigation_id)},
        ...
        result = run_investigation(progress_callback=_progress_callback(db, investigation_id), context=context, security_precomputed=security_precomputed)
