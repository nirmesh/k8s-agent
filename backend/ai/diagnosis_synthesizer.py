...
_SIGNAL_ALIASES = {
    "image": {"image_pull_failure", "image_reference"},
    "pull": {"image_pull_failure", "image_reference"},
    "probe": {"probe_failure", "probe_configuration", "deployment_rollout_failure"},
    "readiness": {"probe_failure", "probe_configuration", "deployment_rollout_failure"},
    "liveness": {"probe_failure", "probe_configuration", "deployment_rollout_failure"},
    "startup": {"probe_failure", "probe_configuration", "deployment_rollout_failure"},
    "schedule": {"scheduling_failure", "scheduling_constraints"},
    "scheduling": {"scheduling_failure", "scheduling_constraints"},
    "service": {"service_routing_failure"},
    "endpoint": {"service_routing_failure"},
    "pvc": {"pvc_binding_failure"},
    "storage": {"pvc_binding_failure"},
    "deployment": {"deployment_rollout_failure"},
    "rollout": {"deployment_rollout_failure"},
...
def _finding_is_grounded(finding: dict[str, Any], by_id: dict[str, dict[str, Any]], valid_resources: set[str]) -> bool:
    ...
    # At least one cited evidence item must belong to an affected resource or
    # explicitly relate to it. This blocks cross-workload contamination.
    for item in cited:
        if item.get("resource") in resources:
            return True
        if set(item.get("related_resources") or []) & set(resources):
            return True
    return False
