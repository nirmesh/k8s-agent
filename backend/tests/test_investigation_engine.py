...
class FakeToolkit:
    ...
    def get_resources(self, kind, namespace=None):
        ...
        if namespace:
            items = [x for x in items if (x.get("metadata") or {}).get("namespace") == namespace]
...
def test_readiness_failure_keeps_probe_and_image_evidence_separate():
    toolkit = FakeToolkit({
        "pod": [{"metadata": {"namespace": "ai-test", "name": "readiness-pod"}, "status": {"phase": "Running", "containerStatuses": [{"name": "nginx", "state": {"running": {}}}]}}],
        "deployment": [{"metadata": {"namespace": "ai-test", "name": "readiness-app"}, "spec": {"replicas": 1}, "status": {"replicas": 1, "readyReplicas": 0, "availableReplicas": 0}}],
        "service": [], "persistentvolumeclaim": [], "endpoints": [],
...
def test_image_pull_failure_requires_image_pull_evidence():
    toolkit = FakeToolkit({"pod": [{"metadata": {"namespace": "default", "name": "broken"}, "status": {"phase": "Pending", "containerStatuses": [{"name": "nginx", "image": "nginx:99.99", "state": {"waiting": {"reason": "ImagePullBackOff"}}}]}}], "events": {"broken": [{"reason": "Failed", "message": 'Failed to pull image "nginx:99.99": manifest unknown'}]}, "deployment": [], "service": [], "persistentvolumeclaim": [], "endpoints": []})
    evidence = collect_operational_evidence(toolkit)
    assert any(e["signal"] == "image_pull_failure" for e in evidence)


def test_service_evidence_records_selector_and_matches():
    toolkit = FakeToolkit({"pod": [], "deployment": [], "service": [{"metadata": {"namespace": "sre-lab", "name": "web-service"}, "spec": {"selector": {"app": "WRONG"}}}], "endpoints": [{"metadata": {"namespace": "sre-lab", "name": "web-service"}, "subsets": []}], "persistentvolumeclaim": []})
