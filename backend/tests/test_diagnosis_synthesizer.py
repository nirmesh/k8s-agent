...
def test_readiness_diagnosis_cannot_use_image_evidence():
    evidence = [
    ...
    result = {
        "findings": [{
            ...
            "root_cause": "Image is wrong",
            "affected_resources": ["Deployment/ai-test/readiness-app"],
            "evidence_ids": ["probe_configuration-1"],
            "confidence": 0.99,
        }]
    }
...
def test_service_diagnosis_must_be_resource_grounded():
    ...
    result = {
        "findings": [{
            "incident_type": "service_selector_mismatch",
            "root_cause": "Selector has no matching pods",
            "affected_resources": ["Service/sre-lab/other-service"],
            "evidence_ids": ["service_routing_failure-1"],
            "confidence": 0.95,
        }]
    }
    validated = validate_diagnosis(result, evidence)
...
def test_multiple_independent_findings_are_preserved():
    evidence = [
        _evidence("probe_failure", "Pod/ai-test/readiness-pod"),
        _evidence("image_pull_failure", "Pod/default/broken-pod"),
    ]
    result = {
        "findings": [
            {
            ...
            },
            {
                ...
                "affected_resources": ["Pod/default/broken-pod"],
                "evidence_ids": ["image_pull_failure-1"],
                "confidence": 0.97,
            },
        ]
    }
