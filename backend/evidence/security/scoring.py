...
def score_security_posture(summary: dict) -> dict:
    """Return a bounded, workload-normalized posture score.

    The score is intentionally not a raw CVE counter. UNKNOWN findings are excluded
    because their severity is not known. Diminishing returns prevent hundreds of
    duplicate/package-level findings from instantly saturating the score.
    """
    ...
    score = max(5, round(100 - penalty)) if known_points > 0 else 100

    summary["cluster_security_score"] = score
    summary["score_basis"] = (
        "Workload-normalized weighted posture score; UNKNOWN findings excluded. "
        "CRITICAL=10, HIGH=4, MEDIUM=1, LOW=0.25, misconfiguration=2, exposed secret=15, "
        "with diminishing returns and a 5/100 floor when known risk exists."
    )
