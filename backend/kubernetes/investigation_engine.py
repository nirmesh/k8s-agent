...
def _resource(kind: str, obj: dict[str, Any]) -> str:
    ...
    return f"{kind}/{meta.get('namespace') or '-'}/{meta.get('name', 'unknown')}"
...
def _evidence(signal: str, resource: str, payload: dict[str, Any], severity: str = "MEDIUM", related: list[str] | None = None) -> dict[str, Any]:
    return {"id": _stable_id(signal, resource, payload), "provider": "kubernetes", "type": "operational_signal", "signal": signal, "resource": resource, "severity": severity, "related_resources": related or [], "payload": payload}


def _event_matches(events: list[dict[str, Any]], tokens: tuple[str, ...]) -> list[dict[str, Any]]:
    matches = []
    for event in events:
        haystack = f"{event.get('reason') or ''} {event.get('message') or ''}".lower()
...
def collect_operational_evidence(toolkit: K8sToolkit, limit: int = 100) -> list[dict[str, Any]]:
    ...
    def add(item: dict[str, Any]) -> None:
        if item["id"] not in seen and len(evidence) < limit:
            seen.add(item["id"])
    ...
    for pod in pods:
        ...
        if phase not in TERMINAL_OK or failures or image_events or probe_events or schedule_events:
            ...
            for owner_resource in owners:
                ...
                template_spec = ((owner_obj.get("spec") or {}).get("template") or {}).get("spec") or {}
                containers = template_spec.get("containers") or []
                if signal == "image_pull_failure":
                    add(_evidence("image_reference", owner_resource, {"containers": [{"name": c.get("name"), "image": c.get("image")} for c in containers]}, "HIGH", [pod_resource]))
                elif signal == "probe_failure":
                    probes = [{"name": c.get("name"), "readiness": c.get("readiness_probe") or c.get("readinessProbe"), "liveness": c.get("liveness_probe") or c.get("livenessProbe"), "startup": c.get("startup_probe") or c.get("startupProbe")} for c in containers]
                    add(_evidence("probe_configuration", owner_resource, {"containers": probes}, "HIGH", [pod_resource]))
                elif signal == "scheduling_failure":
                    add(_evidence("scheduling_constraints", owner_resource, {"node_selector": template_spec.get("node_selector") or template_spec.get("nodeSelector"), "affinity": template_spec.get("affinity"), "tolerations": template_spec.get("tolerations")}, "HIGH", [pod_resource]))
    ...
    for dep in deployments:
        ...
        if ready != desired or available != desired or any(c.get("type") == "Progressing" and c.get("reason") == "ProgressDeadlineExceeded" for c in conditions):
    ...
    for svc in services:
        ...
        if not matching_pods or not addresses:
            add(_evidence("service_routing_failure", f"Service/{ns}/{name}", {"selector": selector, "matching_pods": [_resource("Pod", p) for p in matching_pods], "ready_endpoint_count": len(addresses), "not_ready_endpoint_count": len(not_ready), "endpoint_resource": f"Endpoints/{ns}/{name}"}, "HIGH", [_resource("Pod", p) for p in matching_pods] + [f"Endpoints/{ns}/{name}"]))
    ...
    for pvc in pvcs:
        ...
        add(_evidence("pvc_binding_failure", f"PersistentVolumeClaim/{ns}/{name}", {"phase": status.get("phase"), "warning_events": [{"reason": e.get("reason"), "message": e.get("message")} for e in events]}, "HIGH"))
