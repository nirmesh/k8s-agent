              ...
              <div className="space-y-4 px-5 py-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium leading-6 text-slate-200">
                    {finding.root_cause || "No root cause description provided."}
                  </p>
                </div>
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <header className="flex items-center justify-between rounded-2xl border border-slate-700/30 bg-slate-900/60 px-6 py-4 shadow-lg backdrop-blur">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <button onClick={logout} className="rounded-lg px-3 py-2 text-sm font-medium text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-100">Log out</button>
          </div>
        </header>
        ...
        {loadingClusters ? (
          ...
          <section className="rounded-2xl border border-slate-700/30 bg-slate-900/60 p-6 shadow-lg backdrop-blur">
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400">Select Cluster</label>
            <select value={selectedContext} onChange={(e) => setSelectedContext(e.target.value)} className="mt-3 w-full rounded-xl border border-slate-600 bg-slate-950 px-4 py-3 text-slate-100 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500">
              {clusters.map((c) => <option key={c.name} value={c.name} className="bg-slate-950 text-slate-100">{c.name} — {c.server || c.cluster_name}</option>)}
            </select>
          </section>
        )}

        <button onClick={startInvestigation} disabled={investigating || loadingClusters || clusters.length === 0} className="w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-6 py-3.5 text-center font-semibold text-white shadow-lg shadow-cyan-900/20 transition-all hover:from-cyan-400 hover:to-blue-500 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <section className="rounded-2xl border border-slate-700/30 bg-slate-900/60 p-6 shadow-lg backdrop-blur">
            ...
            <ul className="space-y-3">
              {(() => {
                ...
                const highestCompleted = ALL_STEPS.reduce((max, step, idx) => reported.has(step) ? idx : max, -1);
                ...
                const activeIndex = investigating ? Math.min(completedIndex + 1, ALL_STEPS.length - 1) : -1;
                return ALL_STEPS.map((step, index) => {
                  const isCompleted = completed.has(step);
                  const isActive = index === activeIndex && !isCompleted;
                  return (
                    <li key={step} className={isCompleted ? "flex items-center gap-4 rounded-xl bg-emerald-500/10 px-4 py-3 text-emerald-100" : isActive ? "flex items-center gap-4 rounded-xl bg-blue-500/10 px-4 py-3 text-blue-200" : "flex items-center gap-4 rounded-xl px-4 py-3 text-slate-500"}>
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-700/30 bg-slate-900/60 shadow-lg backdrop-blur">
            <div className="flex items-center justify-between border-b border-slate-700/30 bg-gradient-to-r from-rose-500/10 to-orange-500/10 px-6 py-4">
              <div className="flex items-center gap-3">
                <svg className="h-6 w-6 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <div className="flex flex-col"><h2 className="text-lg font-semibold text-slate-100">Diagnosis</h2><span className="text-xs text-slate-500">Cluster-wide operational findings</span></div>
              </div>
              <span className="rounded-full bg-blue-500/20 px-3 py-1 text-sm font-semibold text-blue-300">{Math.round((current.diagnosis.confidence || 0) * 100)}% confidence</span>
            </div>

            <div className="space-y-6 p-6">
              <DiagnosisFindings diagnosis={current.diagnosis} />

              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Suggested Fix</p>
                <div className="mt-2 text-slate-300"><FixDisplay fix={current.diagnosis.fix} /></div>
              </div>

              <RemediationPanel remediationId={current.remediation_id} investigation={current} onUpdate={setCurrent} />

              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Kubectl Command</p>
                <div className="mt-2 overflow-x-auto rounded-lg border border-slate-700 bg-slate-950 p-4 font-mono text-sm text-emerald-400">{current.diagnosis.kubectl_command || "No command suggested."}</div>
              </div>
              <div>
