    ...
    <section className="overflow-hidden rounded-2xl border border-slate-700/30 bg-slate-900/60 shadow-lg backdrop-blur">
      ...
      {!expandedSection && (
        <div className="flex flex-wrap items-center gap-2 border-t border-slate-700/20 px-6 py-3 text-xs text-slate-500">
          <span>{summary.total_vulnerabilities ?? 0} vulnerabilities</span>
          <span>·</span>
            ...
            {top.length === 0 ? <p className="text-sm text-slate-500">No security findings collected.</p> : top.map((w, idx) => {
              ...
              return (
                <div key={idx} className="rounded-xl border border-slate-700/30 bg-slate-950/50 p-4">
                  <button onClick={() => toggleWorkload(idx)} className="flex w-full items-center justify-between text-left">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex gap-2">{SEVERITY_ORDER.map((s) => w.counts?.[s] ? <span key={s} className={`rounded-full px-2 py-0.5 text-xs font-semibold ${s === "CRITICAL" ? "bg-rose-500/20 text-rose-300" : s === "HIGH" ? "bg-orange-500/20 text-orange-300" : s === "MEDIUM" ? "bg-amber-500/20 text-amber-300" : "bg-emerald-500/20 text-emerald-300"}`}>{s}: {w.counts[s]}</span> : null)}</div>
